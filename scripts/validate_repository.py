...
def validate_rust_review_artifact_contract(text: str, failures: list[str]) -> None:
    ...
    if expected_environment not in validation:
        fail(
            f"{path_text}: canonical Rust validation must receive the fixed review-artifact directory",
            failures,
        )
    ...
    expected_artifact_fragments = (
        ...
        "name: rust-repository-review-artifacts-${{ steps.revision.outputs.expected_revision }}",
        "path: ${{ runner.temp }}/repository-review-artifacts",
        "if-no-files-found: ignore",
        "retention-days: 7",
    )
    for fragment in expected_artifact_fragments:
        if fragment not in artifact:
            fail(f"{path_text}: review artifact contract missing {fragment!r}", failures)
    ...
    if text.count(
        ...
        fail(f"{path_text}: review artifact name must be exact and unique", failures)
    upload_pin = (
